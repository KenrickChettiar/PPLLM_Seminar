...
import Qwen #importing model file which creates responses
import judgeLlama3model #import the judge Qwen
import json
...
Qwen.generateQwen(all_prompts,"BaseQwen_responses")

with open("BaseQwen_responses.json","r") as f:
    responses = json.load(f)

baseQwenresponses = []

for e in responses:
    baseQwenresponses.append(list(e.values())[0])

#get the prompts used for 10 point scoring. this will be passed to the judges.
scoring_prompts = scoringprompts.scoringprompt(eval_data,baseQwenresponses)

with open("scoringpromptsbaseQwen.json","w") as f:
    json.dump(scoring_prompts,f)

#Goto getbaseQwen10pointscores.py and execute further
